...
class WishReceiptDraft {
  ...
  FinancialTransaction toTransaction({required String persistedImagePath}) {
    ...
    return FinancialTransaction(
      ...
      notes: 'Whish receipt shared ${sharedAt.toLocal().toIso8601String()}',
      raw: raw,
    );
  }
}
...
class WishReceiptImportService {
  ...
  Future<String> persistImage(WishReceiptDraft draft) async {
    ...
    final target = Directory(
      '${directory.path}${Platform.pathSeparator}wish_receipts',
    );
    ...
    final output = File(
      '${target.path}${Platform.pathSeparator}wish_$safeId$extension',
    );
  ...
  WishReceiptDraft _parse(String raw, String imagePath, DateTime sharedAt) {
    ...
    final id = _match(
      text,
      RegExp(r'TRANSACTION\s*ID\s*:\s*([0-9]+)', caseSensitive: false),
    );
    final date = _parseDate(
      _match(text, RegExp(r'(20\d{2}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})')),
    );
    ...
    final amountMatch = kind == WishReceiptKind.exchange
        ? RegExp(
            r'TO\s*AMOUNT\s*:\s*([\d,.]+)\s*(USD|LBP)',
            caseSensitive: false,
          ).firstMatch(text)
        : RegExp(
            r'AMOUNT\s*:\s*([\d,.]+)\s*(USD|LBP)',
            caseSensitive: false,
          ).firstMatch(text);
    ...
    final rate = _number(
      _match(
        text,
        RegExp(r'1\s*USD\s*[↔⇄<=>-]+\s*([\d,.]+)\s*LBP', caseSensitive: false),
      ),
    );
    final merchant = _match(
      text,
      RegExp(r'MERCHANT\s*NAME\s*:\s*(.+)', caseSensitive: false),
    );
    final phone = _match(
      text,
      RegExp(
        r'(?:RECEIVER\s*PHONE\s*NUMBER|\+961)\s*:?\s*(\+?\d+)',
        caseSensitive: false,
      ),
    );
    final hasPlus = RegExp(
      r'\+\s*[\d,.]+\s*(?:USD|LBP)',
      caseSensitive: false,
    ).hasMatch(text);
    final defaultType = switch (kind) {
      ...
      WishReceiptKind.transfer =>
        hasPlus ? TransactionType.income : TransactionType.expense,
      WishReceiptKind.unknown => TransactionType.expense,
    };
    final description = switch (kind) {
      WishReceiptKind.exchange => 'Whish exchange: LBP to USD',
      WishReceiptKind.topup => 'Whish Money top up',
      WishReceiptKind.merchant =>
        merchant.isEmpty ? 'Whish merchant payment' : merchant,
      WishReceiptKind.transfer =>
        phone.isEmpty ? 'Whish to Whish transfer' : 'Whish transfer - $phone',
      WishReceiptKind.unknown => 'Whish receipt',
    };
    final category = switch (kind) {
      WishReceiptKind.exchange => 'Currency exchange',
      WishReceiptKind.topup => 'Whish top up',
      WishReceiptKind.merchant => 'Games',
      WishReceiptKind.transfer => hasPlus ? 'Whish received' : 'Whish transfer',
      WishReceiptKind.unknown => 'Whish Money',
    };
  ...
  String _match(String input, RegExp expression) =>
      expression.firstMatch(input)?.group(1)?.trim() ?? '';
